...
void InputCreator::makePoints(int pointNum, Distribution dist,
                              Point2HVec &pointVec, int seed) {
  ...
  for (int i = 0; i < pointNum; ++i) {
    do {
      ...
      case TwoLineDistribution: {
        const Point2 L[2][2] = {{{0.0, 0.0}, {0.3, 0.5}},
                                {{0.7, 0.5}, {1.0, 1.0}}};
...
void InputCreator::readPoints(std::string inFilename, Point2HVec &pointVec,
                              SegmentHVec &constraintVec,
                              int maxConstraintNum) {
  bool isBinary =
      (0 != inFilename.substr(inFilename.length() - 4, 4).compare(".txt"));
  ...
  if (isBinary) {
    ...
    if (0 != bufferSize % pointNum) {
      std::cout << "Invalid input file format! Wrong file size: " << bufferSize
                << std::endl;
      exit(-1);
    }
  ...
  if (dupConstraint > 0)
    std::cout << dupConstraint
              << " degenerate or ignored constraints in input file!"
              << std::endl;
}
