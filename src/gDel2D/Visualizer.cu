...
Visualizer::Visualizer() : _colorMap(_hashUInt) {
  // Default window size
  _winSize = 768;
  _winWidth = _winSize;
  _winHeight = _winSize;
...
void Visualizer::disable() { _enable = false; }

bool Visualizer::isEnable() { return _enable; }

void Visualizer::pause() { _paused = true; }

void Visualizer::resume() { _paused = false; }
...
GLuint Visualizer::getColor(int id) {
  ...
  if (id >= 0) {
    if (!_colorMap.get(id, &color)) {
      GLubyte newColor[4] = {GLubyte(randGen.getNext() * 255),
                             GLubyte(randGen.getNext() * 255),
                             GLubyte(randGen.getNext() * 255), 64};
...
void Visualizer::refresh() {
  ...
  glBufferData(GL_ARRAY_BUFFER, _pointNum * sizeof(PointAttribute), glPoints,
               GL_STATIC_DRAW);
  ...
  for (unsigned int i = 0; i < _triNum; i++) {
    ...
    for (int j = 0; j < 3; j++)
      if (!tri.has(_infIdx)) {
        glTriangles[i * 3 + j].x =
            (float)_curPointVec[_curTriVec[i]._v[j]]._p[0];
        glTriangles[i * 3 + j].y =
            (float)_curPointVec[_curTriVec[i]._v[j]]._p[1];
  ...
  GLubyte col[2][4] = {{81, 31, 82, 255}, {243, 110, 244, 255}};

  for (unsigned int i = 0; i < _constraintNum; i++)
    for (int j = 0; j < 2; j++) {
      glConstraints[i * 2 + j].x =
          (float)_curPointVec[_curConstraintVec[i]._v[j]]._p[0];
      glConstraints[i * 2 + j].y =
          (float)_curPointVec[_curConstraintVec[i]._v[j]]._p[1];
...
void Visualizer::drawClicked() {
  ...
  if (_clickedId >= PointMask) {
    ...
    for (int i = 0; i < 2; i++) {
      ...
      glVertex2f((float)_curPointVec[idx]._p[0],
                 (float)_curPointVec[idx]._p[1]);
      tip = idx;
    }
    glEnd();
...
void Visualizer::click() {
  ...
  if (_clickedId > 0) {
    ...
    switch (type) {
    case PointMask:
      printf("Vertex %i (%.10f %.10f)\n", id, _curPointVec[id]._p[0],
             _curPointVec[id]._p[1]);
      break;
    case EdgeMask:
      printf("Constraint %i (%i %i)\n", id, _curConstraintVec[id]._v[0],
             _curConstraintVec[id]._v[1]);
      break;
    case TriMask:
      printf("Triangle %i (%i %i %i)\n", id, _curTriVec[id]._v[0],
             _curTriVec[id]._v[1], _curTriVec[id]._v[2]);
      break;
    }
  } else
...
void Visualizer::nextFrame() {
  if (_curFrame + 1 >= _frameList.size())
    return;
...
void Visualizer::addFrame(const Point2HVec &pointVec,
                          const SegmentHVec &constraintVec,
                          const TriHVec &triVec, const IntHVec &triColorVec,
                          int infIdx) {
  if (!_enable || _paused)
    return;
  ...
  //     << ": " << (int) (sizeInBytes / 1024.0 / 1024.0 * 100 ) / 100.0 << "MB"
  //     << std::endl;
...
void Visualizer::addFrame(const Point2HVec &pointVec,
                          const SegmentHVec &constraintVec,
                          const TriHVec &triVec, int infIdx) {
  addFrame(pointVec, constraintVec, triVec, IntHVec(), infIdx);
}

void Visualizer::addFrame(const Point2DVec &pointVec, const TriDVec &triVec,
                          int infIdx) {
  if (!_enable || _paused)
    return;
...
void Visualizer::addFrame(const Point2DVec &pointVec,
                          const SegmentDVec &constraintVec,
                          const TriDVec &triVec, int infIdx) {
  if (!_enable || _paused)
    return;
...
void Visualizer::addFrame(const Point2DVec &pointVec,
                          const SegmentDVec &constraintVec,
                          const TriDVec &triVec, const IntHVec &triColorVec,
                          int infIdx) {
  if (!_enable || _paused)
    return;
...
void Visualizer::zoomToFit() {
  ...
  for (int i = 0; i < _curPointVec.size(); ++i) {
    if (_curPointVec[i]._p[0] < minX)
      minX = _curPointVec[i]._p[0];
    if (_curPointVec[i]._p[0] > maxX)
      maxX = _curPointVec[i]._p[0];
    if (_curPointVec[i]._p[1] < minY)
      minY = _curPointVec[i]._p[1];
    if (_curPointVec[i]._p[1] > maxY)
      maxY = _curPointVec[i]._p[1];
  }
