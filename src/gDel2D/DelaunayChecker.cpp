...
int DelaunayChecker::getTriangleCount() { return _output.triVec.size(); }
...
void DelaunayChecker::checkEuler() {
  ...
  std::cout << "Euler check: " << ((1 != euler) ? " ***Fail***" : " Pass")
            << std::endl;
...
void printTriAndOpp(int ti, const Tri &tri, const TriOpp &opp) {
  printf("triIdx: %d [ %d %d %d ] ( %d:%d %d:%d %d:%d )\n", ti, tri._v[0],
         tri._v[1], tri._v[2], opp.getOppTri(0), opp.getOppVi(0),
         opp.getOppTri(1), opp.getOppVi(1), opp.getOppTri(2), opp.getOppVi(2));
}
  ...
  for (int ti0 = 0; ti0 < (int)triVec.size(); ++ti0) {
    ...
    for (int vi = 0; vi < DEG; ++vi) {
      ...
      if (ti0 != opp1.getOppTri(vi0_1)) {
        std::cout << "Not opp of each other! Tri0: " << ti0 << " Tri1: " << ti1
                  << std::endl;
      ...
      if (vi != opp1.getOppVi(vi0_1)) {
        std::cout << "Vi mismatch! Tri0: " << ti0 << "Tri1: " << ti1
                  << std::endl;
        continue;
      }
    }
...
void DelaunayChecker::checkOrientation() {
  ...
  for (int i = 0; i < (int)triVec.size(); ++i) {
    ...
    const Orient ord =
        _predWrapper.doOrient2DFastExactSoS(t._v[0], t._v[1], t._v[2]);
...
void DelaunayChecker::checkConstraints() {
  if (_input.constraintVec.size() == 0)
    return;
  ...
  for (int i = 0; i < consVec.size(); ++i) {
    ...
    if (j == INT_MAX) // Found
    {
      ...
      if (j >= MaxWalking)
        std::cout << "Vertex degree too high; Skipping constraint " << i
                  << std::endl;
      // else
