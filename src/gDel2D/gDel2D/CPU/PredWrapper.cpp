...
int PredWrapper2D::pointNum() const { return _pointNum + 1; }
...
Orient PredWrapper2D::doOrient2DSoSOnly(const RealType *p0, const RealType *p1,
                                        const RealType *p2, int v0, int v1,
                                        int v2) const {
  ////
...
Side PredWrapper2D::doIncircle(Tri tri, int vert) const {
  ...
  const Point2 pt[] = {getPoint(tri._v[0]), getPoint(tri._v[1]),
                       getPoint(tri._v[2]), getPoint(vert)};
