...
enum Counter { CounterExact, CounterFlag, CounterNum };

///////////////////////////////////////////////////////////////////// Orient //

enum Orient { OrientNeg = -1, OrientZero = +0, OrientPos = +1 };
...
enum Side { SideIn = -1, SideZero = +0, SideOut = +1 };
...
struct Point2 {
  ...
  INLINE_H_D bool operator<(const Point2 &pt) const { return lessThan(pt); }

  INLINE_H_D bool equals(const Point2 &pt) const {
    return (_p[0] == pt._p[0] && _p[1] == pt._p[1]);
  }

  INLINE_H_D bool operator==(const Point2 &pt) const { return equals(pt); }
...
struct Tri {
  ...
  INLINE_H_D bool operator==(const Tri &pt) const { return equals(pt); }
...
INLINE_H_D bool isValidTriVi(int vi) { return (vi >= 0 && vi < DEG); }
...
template <typename T> INLINE_H_D bool isBitSet(T c, int bit) {
  return (1 == ((c >> bit) & 1));
}

template <typename T> INLINE_H_D void setBitState(T &c, int bit, bool state) {
  const T val = (1 << bit);
  c = state ? (c | val) : (c & ~val);
}
...
INLINE_H_D int getOppValTriVi(int val) { return (val & ~0x08); }

INLINE_H_D int getOppValTri(int val) { return (val >> 4); }

INLINE_H_D int getOppValVi(int val) { return (val & 3); }

INLINE_H_D bool isOppValConstraint(int val) { return isBitSet(val, 2); }
...
struct TriOpp {
  ...
  INLINE_H_D bool isNeighbor(int triIdx) const {
    return ((_t[0] >> 4) == triIdx || (_t[1] >> 4) == triIdx ||
            (_t[2] >> 4) == triIdx);
  }
  ...
  INLINE_H_D bool isOppSpecial(int vi) const { return isBitSet(_t[vi], 3); }
  ...
  INLINE_H_D int getOppTri(int vi) const { return getOppValTri(_t[vi]); }

  INLINE_H_D void setOppTri(int vi, int idx) {
    return setOppValTri(_t[vi], idx);
  }

  INLINE_H_D int getOppVi(int vi) const { return getOppValVi(_t[vi]); }
};
...
enum PairType { PairNone = 0, PairSingle = 1, PairDouble = 2, PairConcave = 3 };

INLINE_H_D bool isTriAlive(char c) { return isBitSet(c, 0); }

INLINE_H_D void setTriAliveState(char &c, bool b) { setBitState(c, 0, b); }
...
INLINE_H_D PairType getTriPairType(char c) { return (PairType)((c >> 2) & 3); }
...
struct Segment {
  ...
  INLINE_H_D bool operator==(const Segment &seg) const { return equals(seg); }

  INLINE_H_D bool operator!=(const Segment &pt) const { return !equals(pt); }
  ...
  INLINE_H_D bool operator<(const Segment &seg) const { return lessThan(seg); }
...
INLINE_H_D int getConsFlipVoteIdx(int vote) { return (vote & 0x1FFFFFFF); }
...
enum CheckDelaunayMode { CircleFastOrientFast, CircleExactOrientSoS };

enum ActTriMode { ActTriMarkCompact, ActTriCollectCompact };
