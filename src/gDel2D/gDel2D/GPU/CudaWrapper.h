...
template <typename T> T *cuNew(int num) {
  T *loc = NULL;
...
template <typename T> void cuDelete(T **loc) {
  CudaSafeCall(cudaFree(*loc));
  *loc = NULL;
  return;
}

template <typename T> __forceinline__ __device__ void cuSwap(T &v0, T &v1) {
  const T tmp = v0;
  v0 = v1;
  v1 = tmp;
...
inline void cuPrintMemory(const char *inStr) {
  ...
  printf("[%s] Memory used: %zu MB\n", inStr, (total - free) / MegaByte);

  return;
}

// Obtained from: C:\ProgramData\NVIDIA Corporation\GPU
// SDK\C\common\inc\cutil_inline_runtime.h This function returns the best GPU
// (with maximum GFLOPS)
