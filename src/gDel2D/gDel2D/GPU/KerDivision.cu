...
template <typename T> __forceinline__ __device__ T min(T a, T b) {
  if (a < b)
    return a;
  else
    return b;
...
__global__ void kerSplitTri(KerIntArray splitTriArr, Tri *triArr,
                            TriOpp *oppArr, char *triInfoArr, int *insTriMap,
                            int *triToVert, int triNum, int insTriNum) {
  // Iterate current triangles
  for (int idx = getCurThreadIdx(); idx < splitTriArr._num;
       idx += getThreadNum()) {
    ...
    for (int ni = 0; ni < DEG; ++ni) {
      ...
      if (-1 == neiNewBeg) // Neighbour is un-split
      {
        oppArr[neiTriIdx].setOpp(
            neiTriVi, newTriIdx[ni],
            2); // Point un-split neighbour back to this new triangle
      } else    // Neighbour has split
      {
        // Get neighbour's new split triangle that has this face
        if (triNum >= 0)
          neiTriIdx =
              ((0 == neiTriVi) ? neiTriIdx
      ...
      newOpp[ni].setOpp(2, neiTriIdx,
                        neiTriVi); // Point this triangle to neighbour
    }
    ...
    for (int ti = 0; ti < DEG; ++ti) {
      const Tri newTri = {tri._v[(ti + 1) % DEG], tri._v[(ti + 2) % DEG],
                          splitVertex};
...
__global__ void kerMarkRejectedFlips(int *actTriArr, TriOpp *oppArr,
                                     int *triVoteArr, char *triInfoArr,
                                     int *flipToTri, int actTriNum,
                                     int *dbgRejFlipArr) {
...
__global__ void kerFlip(KerIntArray flipToTri, Tri *triArr, TriOpp *oppArr,
                        char *triInfoArr, int2 *triMsgArr, int *actTriArr,
                        FlipItem *flipArr, int *triConsArr, int *vertTriArr,
                        int orgFlipNum, int actTriNum) {
  // Iterate flips
  for (int flipIdx = getCurThreadIdx(); flipIdx < flipToTri._num;
    ...
    if (actTriArr != NULL)
      actTriArr[actTriNum + flipIdx] =
          (Checked == getTriCheckState(triInfoArr[topIdx])) ? topIdx : -1;
...
__global__ void kerUpdateOpp(FlipItem *flipVec, TriOpp *oppArr, int2 *triMsgArr,
                             int *flipToTri, int orgFlipNum, int flipNum) {
  // Iterate flips
...
__global__ void kerUpdateFlipTrace(FlipItem *flipArr, int *triToFlip,
                                   int orgFlipNum, int flipNum) {
...
__global__ void kerUpdateVertIdx(KerTriArray triVec, char *triInfoArr,
                                 int *orgPointIdx) {
...
__global__ void kerShiftTriIdx(KerIntArray idxVec, int *shiftArr) {
...
__global__ void kerMarkSpecialTris(KerCharArray triInfoVec, TriOpp *oppArr) {
  for (int idx = getCurThreadIdx(); idx < triInfoVec._num;
       idx += getThreadNum()) {
...
__global__ void kerPickWinnerPoint(KerIntArray vertexTriVec, int *vertCircleArr,
                                   int *triCircleArr, int *triVertArr,
                                   int noSample) {
...
__global__ void kerMakeFirstTri(Tri *triArr, TriOpp *oppArr, char *triInfoArr,
                                Tri tri, int infIdx) {
...
__global__ void kerShiftOpp(KerIntArray shiftVec, TriOpp *src, TriOpp *dest,
                            int destSize) {
...
__global__ void kerMarkInfinityTri(KerTriArray triVec, char *triInfoArr,
                                   TriOpp *oppArr, int infIdx) {
  for (int idx = getCurThreadIdx(); idx < triVec._num; idx += getThreadNum()) {
...
__global__ void kerCollectFreeSlots(char *triInfoArr, int *prefixArr,
                                    int *freeArr, int newTriNum) {
...
__global__ void kerMakeCompactMap(KerCharArray triInfoVec, int *prefixArr,
                                  int *freeArr, int newTriNum) {
  for (int idx = newTriNum + getCurThreadIdx(); idx < triInfoVec._num;
       idx += getThreadNum()) {
    if (!isTriAlive(triInfoVec._arr[idx])) {
      prefixArr[idx] = -1;
      continue;
...
__global__ void kerCompactTris(KerCharArray triInfoVec, int *prefixArr,
                               Tri *triArr, TriOpp *oppArr, int newTriNum) {
  for (int idx = newTriNum + getCurThreadIdx(); idx < triInfoVec._num;
       idx += getThreadNum()) {
...
__global__ void kerMarkRejectedConsFlips(KerIntArray actTriVec, int *triConsArr,
                                         int *triVoteArr, char *triInfoArr,
                                         TriOpp *oppArr, int *flipToTri,
                                         int *dbgRejFlipArr) {
    ...
    if (INT_MAX != voteVal) {
      ...
      if (bossTriIdx == midIdx) // Boss of myself
      {
        ...
        if (triVoteArr[rightIdx] == voteVal) {
          ...
          else if (triVoteArr[leftIdx] == voteVal)
            output = encode(midIdx, midVi);
          ...
          if (output != -1) {
            ...
            {
              const int nextIdx =
                  oppArr[rightIdx].getOppTri(decode_cVi(rightLabel));
