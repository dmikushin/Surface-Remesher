...
void DPredWrapper::init(Point2 *pointArr, int pointNum, int *orgPointIdx,
                        int infIdx) {
  _pointArr = pointArr;
  _pointNum = pointNum;
...
void DPredWrapper::cleanup() { cuDelete(&_predConsts); }
...
__forceinline__ __device__ Orient DPredWrapper::doOrient2DFast(int v0, int v1,
                                                               int v2) const {
...
__forceinline__ __device__ Orient DPredWrapper::doOrient2DSoSOnly(
    const RealType *p0, const RealType *p1, const RealType *p2, int v0, int v1,
    int v2) const {
  ////
...
DPredWrapper::doOrient2DFastExactSoS(int v0, int v1, int v2) const {
  const RealType *pt[] = {getPoint(v0)._p, getPoint(v1)._p, getPoint(v2)._p};
...
__forceinline__ __device__ Side DPredWrapper::doInCircleFast(Tri tri,
                                                             int vert) const {
  const RealType *pt[] = {getPoint(tri._v[0])._p, getPoint(tri._v[1])._p,
                          getPoint(tri._v[2])._p, getPoint(vert)._p};
  ...
  if (tri.has(_infIdx)) {
    ...
    det = orient2dFast(_predConsts, pt[(infVi + 1) % 3], pt[(infVi + 2) % 3],
                       pt[3]);
  } else
...
DPredWrapper::doOrient2DExact_Lifted(const RealType *p0, const RealType *p1,
                                     const RealType *p2, bool lifted) const {
...
__forceinline__ __device__ Side DPredWrapper::doInCircleSoSOnly(
    const RealType *p0, const RealType *p1, const RealType *p2,
    const RealType *p3, int v0, int v1, int v2, int v3) const {
  ////
  // Sort points using vertex as key, also note their sorted order
  ////
...
__forceinline__ __device__ Side
DPredWrapper::doInCircleFastExactSoS(Tri tri, int vert) const {
  if (vert == _infIdx)
    return SideOut;
  ...
  if (tri.has(_infIdx)) {
    ...
    const Orient ort = doOrient2DFastExactSoS(tri._v[(infVi + 1) % 3],
                                              tri._v[(infVi + 2) % 3], vert);
  ...
  const Side s1 = doInCircleSoSOnly(pt[0], pt[1], pt[2], pt[3], tri._v[0],
                                    tri._v[1], tri._v[2], vert);

  return s1;
}

__forceinline__ __device__ float DPredWrapper::inCircleDet(Tri tri,
                                                           int vert) const {
  const RealType *pt[] = {getPoint(tri._v[0])._p, getPoint(tri._v[1])._p,
                          getPoint(tri._v[2])._p, getPoint(vert)._p};
  ...
  if (tri.has(_infIdx)) {
    ...
    det = orient2dDet(_predConsts, pt[(infVi + 1) % 3], pt[(infVi + 2) % 3],
                      pt[3]);
  } else
