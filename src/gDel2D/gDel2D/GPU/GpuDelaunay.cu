...
RealType orient2dzero(const RealType *pa, const RealType *pb,
                      const RealType *pc);
...
void GpuDel::constructInitialTriangles() {
  ...
  thrust::pair<Point2DIter, Point2DIter> ret =
      thrust::minmax_element(_pointVec.begin(), _pointVec.end(), CompareX());
  ...
  thrust::transform(_pointVec.begin(), _pointVec.end(), distVec.begin(),
                    Get2Ddist(p0, p1));

  const int v2 =
      thrust::max_element(distVec.begin(), distVec.end()) - distVec.begin();
  const Point2 p2 = _pointVec[v2];

  _memPool.release(distVec);

  if (_input->isProfiling(ProfDebug)) {
    std::cout << "Leftmost: " << v0 << " --> " << p0._p[0] << " " << p0._p[1]
              << std::endl;
    std::cout << "Rightmost: " << v1 << " --> " << p1._p[0] << " " << p1._p[1]
              << std::endl;
    std::cout << "Furthest 2D: " << v2 << " --> " << p2._p[0] << " " << p2._p[1]
              << std::endl;
  }
  ...
  if (_input->isProfiling(ProfDiag)) {
    std::cout << "Kernel: " << _ptInfty._p[0] << " " << _ptInfty._p[1]
              << std::endl;
  }
  ...
  kerMakeFirstTri<<<1, 1>>>(toKernelPtr(_triVec), toKernelPtr(_oppVec),
                            toKernelPtr(_triInfoVec), firstTri, _infIdx);
  CudaCheckError();
  ...
  kerInitPointLocationFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_vertTriVec), toKernelPtr(exactCheckVec), _counters.ptr(),
      firstTri);

  kerInitPointLocationExact<<<PredBlocksPerGrid, PredThreadsPerBlock>>>(
      toKernelPtr(_vertTriVec), toKernelPtr(exactCheckVec), _counters.ptr(),
      firstTri);
  CudaCheckError();
...
void GpuDel::initForConstraintInsertion() {
  if (!_input->noSort) {
    ...
    thrust::gather(segInt, segInt + _constraintVec.size() * 2, mapVec.begin(),
                   segInt);
    ...
    //    thrust::transform( _constraintVec.begin(), _constraintVec.end(),
    //    keyVec.begin(), GetConstraintMinVert() );

    //    thrust::sort_by_key( keyVec.begin(), keyVec.end(),
    //    _constraintVec.begin() );
  ...
  kerMapTriToVert<<<BlocksPerGrid, ThreadsPerBlock>>>(toKernelArray(_triVec),
...
bool GpuDel::markIntersections() {
  ...
  kerMarkTriConsIntersectionFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_actConsVec), toKernelPtr(_constraintVec),
      toKernelPtr(_triVec), toKernelPtr(_oppVec), toKernelPtr(_triInfoVec),
      toKernelPtr(_vertTriVec), toKernelPtr(_triConsVec), _counters.ptr());
...
void GpuDel::updatePairStatus() {
  ...
  kerUpdatePairStatusFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_actTriVec), toKernelPtr(_triConsVec), toKernelPtr(_triVec),
      toKernelPtr(_oppVec), toKernelPtr(_triInfoVec), toKernelPtr(exactVec),
      _counters.ptr());
...
void GpuDel::checkConsFlipping(IntDVec &triVoteVec) {
  ...
  kerCheckConsFlippingFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_actTriVec), toKernelPtr(_triConsVec),
      toKernelPtr(_triInfoVec), toKernelPtr(_triVec), toKernelPtr(_oppVec),
      toKernelPtr(triVoteVec), toKernelPtr(exactVec), _counters.ptr());
...
#pragma endregion
  ...
  kerMarkRejectedConsFlips<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_actTriVec), toKernelPtr(_triConsVec),
      toKernelPtr(triVoteVec), toKernelPtr(_triInfoVec), toKernelPtr(_oppVec),
...
#pragma endregion
  ...
  kerFlip<<<BlocksPerGrid, 32>>>(
      toKernelArray(flipToTri), toKernelPtr(_triVec), toKernelPtr(_oppVec),
      NULL, toKernelPtr(_triMsgVec), NULL, toKernelPtr(_flipVec),
      toKernelPtr(_triConsVec), toKernelPtr(_vertTriVec), orgFlipNum, 0);
  CudaCheckError();
  ...
  kerUpdateOpp<<<BlocksPerGrid, 32>>>(
      toKernelPtr(_flipVec) + orgFlipNum, toKernelPtr(_oppVec),
      toKernelPtr(_triMsgVec), toKernelPtr(flipToTri), orgFlipNum, flipNum);
  CudaCheckError();
...
void GpuDel::doInsertConstraints() {
  ...
  while (markIntersections()) {
    ...
    if (Visualizer::instance()->isEnable()) {
      ...
      Visualizer::instance()->addFrame(_pointVec, _constraintVec, _triVec,
                                       triColorVec, _infIdx);
...
void GpuDel::markSpecialTris() {
  ...
  kerMarkSpecialTris<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_triInfoVec), toKernelPtr(_oppVec));
  CudaCheckError();
...
void GpuDel::splitTri() {
  ...
  kerVoteForPoint<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_vertTriVec), toKernelPtr(_triVec),
      toKernelPtr(vertCircleVec), toKernelPtr(triCircleVec), noSample);
  CudaCheckError();
  ...
  kerPickWinnerPoint<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_vertTriVec), toKernelPtr(vertCircleVec),
      toKernelPtr(triCircleVec), toKernelPtr(triToVert), noSample);
  CudaCheckError();
  ...
  if (_input->isProfiling(ProfDiag)) {
    std::cout << "Insert: " << _insNum << " Tri from: " << triNum
  ...
  kerSplitPointsFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_vertTriVec), toKernelPtr(triToVert), toKernelPtr(_triVec),
      toKernelPtr(insTriMap), toKernelPtr(exactCheckVec), _counters.ptr(),
      triNum, _insNum);

  kerSplitPointsExactSoS<<<PredBlocksPerGrid, PredThreadsPerBlock>>>(
      toKernelPtr(_vertTriVec), toKernelPtr(triToVert), toKernelPtr(_triVec),
      toKernelPtr(insTriMap), toKernelPtr(exactCheckVec), _counters.ptr(),
      triNum, _insNum);
  CudaCheckError();
  ...
  kerSplitTri<<<BlocksPerGrid, 32>>>(
      toKernelArray(splitTriVec), toKernelPtr(_triVec), toKernelPtr(_oppVec),
      toKernelPtr(_triInfoVec), toKernelPtr(insTriMap), toKernelPtr(triToVert),
      triNum, _insNum);
  CudaCheckError();
...
#pragma region Diagnostic
  if (_input->isProfiling(ProfDiag)) {
    ...
    if (orgActNum == 0 ||
        (checkMode != CircleExactOrientSoS &&
...
#pragma endregion
  ...
  if (orgActNum < BlocksPerGrid * ThreadsPerBlock &&
      orgActNum * 2 < _actTriVec.capacity() && orgActNum * 2 < triNum) {
...
#pragma endregion
  ...
  kerMarkRejectedFlips<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelPtr(_actTriVec), toKernelPtr(_oppVec), toKernelPtr(triVoteVec),
      toKernelPtr(_triInfoVec), toKernelPtr(flipToTri), orgActNum,
      _input->isProfiling(ProfDiag) ? toKernelPtr(__rejFlipVec) : NULL);
  CudaCheckError();
...
#pragma region Diagnostic
  if (_input->isProfiling(ProfDiag)) {
    ...
    std::cout << "Acts: " << orgActNum << " Flips: " << flipNum << " ( "
              << rejFlipNum << " )"
              << " circle: " << circleNum << " Exact: "
              << (checkMode == CircleExactOrientSoS ? _counters[CounterExact]
                                                    : -1)
              << std::endl;
...
#pragma endregion
  ...
  kerFlip<<<BlocksPerGrid, 32>>>(
      toKernelArray(flipToTri), toKernelPtr(_triVec), toKernelPtr(_oppVec),
      toKernelPtr(_triInfoVec), toKernelPtr(_triMsgVec),
      (_actTriMode == ActTriCollectCompact) ? toKernelPtr(_actTriVec) : NULL,
      toKernelPtr(_flipVec), NULL, NULL, orgFlipNum, orgActNum);
  CudaCheckError();
...
void GpuDel::dispatchCheckDelaunay(CheckDelaunayMode checkMode, int orgActNum,
                                   IntDVec &triVoteVec) {
  switch (checkMode) {
  case CircleFastOrientFast:
    kerCheckDelaunayFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
        toKernelPtr(_actTriVec), toKernelPtr(_triVec), toKernelPtr(_oppVec),
        toKernelPtr(_triInfoVec), toKernelPtr(triVoteVec), orgActNum,
  ...
  case CircleExactOrientSoS:
    ...
    kerCheckDelaunayExact_Fast<<<BlocksPerGrid, ThreadsPerBlock>>>(
        toKernelPtr(_actTriVec), toKernelPtr(_triVec), toKernelPtr(_oppVec),
        toKernelPtr(_triInfoVec), toKernelPtr(triVoteVec),
        toKernelPtr(exactCheckVi), orgActNum, _counters.ptr(),
        _input->isProfiling(ProfDiag) ? toKernelPtr(__circleCountVec) : NULL);

    kerCheckDelaunayExact_Exact<<<PredBlocksPerGrid, PredThreadsPerBlock>>>(
        toKernelPtr(_triVec), toKernelPtr(_oppVec), toKernelPtr(triVoteVec),
        toKernelPtr(exactCheckVi), _counters.ptr(),
        _input->isProfiling(ProfDiag) ? toKernelPtr(__circleCountVec) : NULL);
    CudaCheckError();
...
__global__ void kerShift(KerIntArray shiftVec, T *src, T *dest) {
  for (int idx = getCurThreadIdx(); idx < shiftVec._num;
       idx += getThreadNum()) {
...
template <typename T>
void GpuDel::shiftExpandVec(IntDVec &shiftVec, DevVector<T> &dataVec,
                            int size) {
  ...
  kerShift<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(shiftVec), toKernelPtr(dataVec), toKernelPtr(tempVec));
  CudaCheckError();
...
void GpuDel::shiftOppVec(IntDVec &shiftVec, TriOppDVec &dataVec, int size) {
  ...
  kerShiftOpp<<<BlocksPerGrid, ThreadsPerBlock>>>(toKernelArray(shiftVec),
                                                  toKernelPtr(dataVec),
                                                  toKernelPtr(tempVec), size);
  CudaCheckError();
...
void GpuDel::shiftTri(IntDVec &triToVert, IntDVec &splitTriVec) {
  ...
  kerShiftTriIdx<<<BlocksPerGrid, ThreadsPerBlock>>>(toKernelArray(_vertTriVec),
                                                     toKernelPtr(shiftVec));
  CudaCheckError();

  kerShiftTriIdx<<<BlocksPerGrid, ThreadsPerBlock>>>(toKernelArray(splitTriVec),
                                                     toKernelPtr(shiftVec));
  CudaCheckError();
...
void GpuDel::relocateAll() {
  ...
  if (_availPtNum > 0) {
    ...
    kerRelocatePointsFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
        toKernelArray(_vertTriVec), toKernelPtr(triToFlip),
        toKernelPtr(_flipVec), toKernelPtr(exactCheckVec), _counters.ptr());

    kerRelocatePointsExact<<<BlocksPerGrid, ThreadsPerBlock>>>(
        toKernelPtr(_vertTriVec), toKernelPtr(triToFlip), toKernelPtr(_flipVec),
        toKernelPtr(exactCheckVec), _counters.ptr());
    CudaCheckError();
...
void GpuDel::compactTris() {
  ...
  kerCollectFreeSlots<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelPtr(_triInfoVec), toKernelPtr(prefixVec), toKernelPtr(freeVec),
      newTriNum);
  CudaCheckError();

  // Make map
  kerMakeCompactMap<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_triInfoVec), toKernelPtr(prefixVec), toKernelPtr(freeVec),
      newTriNum);
  CudaCheckError();

  // Reorder the tets
  kerCompactTris<<<BlocksPerGrid, ThreadsPerBlock>>>(
      toKernelArray(_triInfoVec), toKernelPtr(prefixVec), toKernelPtr(_triVec),
      toKernelPtr(_oppVec), newTriNum);
  CudaCheckError();
