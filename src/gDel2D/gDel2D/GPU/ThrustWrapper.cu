...
public:
  ...
  void freeAll() {
    ...
    for (FreeBlocks::iterator i = freeBlocks.begin(); i != freeBlocks.end();
         ++i) {
      cudaFree(i->second);
      totalSize += i->first;
    }

    for (AllocBlocks::iterator i = allocBlocks.begin(); i != allocBlocks.end();
         ++i) {
      cudaFree(i->first);
      totalSize += i->second;
    }
  ...
  char *allocate(std::ptrdiff_t numBytes) {
    ...
    if (freeBlock != freeBlocks.end()) {
      ...
      try {
        // std::cout << "CachedAllocator: no free block found; calling
        // cudaMalloc " << numBytes << std::endl;

        // allocate memory and convert cuda::pointer to raw pointer
        result = thrust::device_malloc<char>(numBytes).get();
      } catch (std::runtime_error &e) {
        // output an error message and exit
        std::cerr << "thrust::device_malloc failed to allocate " << numBytes
                  << " bytes!" << std::endl;
        exit(-1);
      }
    }
...
void thrust_free_all() { thrustAllocator.freeAll(); }
...
void thrust_transform_GetMortonNumber(DevVector<Point2>::iterator inBeg,
                                      ...
                                      RealType minVal, RealType maxVal) {
  thrust::transform(thrust::cuda::par(thrustAllocator), inBeg, inEnd, outBeg,
                    GetMortonNumber(minVal, maxVal));
}
...
int makeInPlaceMapAndSum(IntDVec &inVec) {
  ...
  thrust::exclusive_scan(thrust::cuda::par(thrustAllocator), inVec.begin(),
                         inVec.end(), inVec.begin());
...
int makeInPlaceIncMapAndSum(IntDVec &inVec) {
  thrust::inclusive_scan(thrust::cuda::par(thrustAllocator), inVec.begin(),
                         inVec.end(), inVec.begin());
...
int compactIfNegative(DevVector<int> &inVec, DevVector<int> &tempVec) {
  ...
  tempVec.erase(thrust::copy_if(thrust::cuda::par(thrustAllocator),
                                inVec.begin(), inVec.end(), tempVec.begin(),
                                IsNotNegative()),
                tempVec.end());
...
void thrust_scan_TriHasVert(IntDVec &inVec, IntDVec &outVec) {
  ...
  thrust::transform_exclusive_scan(
      thrust::cuda::par(thrustAllocator), inVec.begin(), inVec.end(),
      outVec.begin(), MakeKeyFromTriHasVert(), 0, thrust::plus<int>());
}
...
int thrust_copyIf_IsActiveTri(const CharDVec &inVec, IntDVec &outVec) {
  ...
  outVec.erase(thrust::copy_if(thrust::cuda::par(thrustAllocator), first, last,
                               inVec.begin(), outVec.begin(), IsTriActive()),
               outVec.end());
...
int thrust_copyIf_IsNotNegative(const IntDVec &inVec, IntDVec &outVec) {
  ...
  outVec.erase(thrust::copy_if(thrust::cuda::par(thrustAllocator), first, last,
                               inVec.begin(), outVec.begin(), IsNotNegative()),
               outVec.end());
...
int thrust_copyIf_TriHasVert(const IntDVec &inVec, IntDVec &outVec) {
  ...
  outVec.erase(thrust::copy_if(thrust::cuda::par(thrustAllocator), first, last,
                               inVec.begin(), outVec.begin(), IsTriHasVert()),
               outVec.end());
...
void thrust_scatterConstantMap(const IntDVec &inVec, CharDVec &outVec,
                               char value) {
...
void thrust_scan_TriAliveStencil(const CharDVec &inVec, IntDVec &outVec) {
  ...
  thrust::transform_inclusive_scan(thrust::cuda::par(thrustAllocator),
                                   inVec.begin(), inVec.end(), outVec.begin(),
                                   TriAliveStencil(), thrust::plus<int>());
}
