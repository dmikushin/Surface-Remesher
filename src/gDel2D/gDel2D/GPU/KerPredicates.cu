...
template <typename T>
__forceinline__ __device__ void storeIntoBuffer(T *s_buffer, int *s_num,
                                                const T &item) {
...
template <typename T>
__forceinline__ __device__ void flushBuffer(T *s_buffer, int &s_offset,
                                            int &s_num, T *g_output,
                                            int &g_counter, bool force) {
  __syncthreads();
...
splitPoints(int vertex, int &vertTriIdx, int *triToVert, Tri *triArr,
            int *insTriMap, int triNum, int insTriNum) {
  ...
  if (doFast &&
      vertex == splitVertex) // Fast mode, *this* vertex will split its triangle
  {
    vertTriIdx = -1;
    return true;
  }
  ...
  const int newBeg =
      (triNum >= 0) ? (triNum + 2 * insTriMap[triIdx]) : (triIdx + 1);
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    Orient ort = (doFast) ? dPredWrapper.doOrient2DFast(triVert[fv[0]],
                                                        triVert[fv[1]], vertex)
                          : dPredWrapper.doOrient2DFastExactSoS(
                                triVert[fv[0]], triVert[fv[1]], vertex);
...
__global__ void kerSplitPointsFast(KerIntArray vertTriVec, int *triToVert,
                                   Tri *triArr, int *insTriMap,
                                   int *exactCheckArr, int *counter, int triNum,
                                   int insTriNum) {
  // Iterate points
  for (int idx = getCurThreadIdx(); idx < vertTriVec._num;
       idx += getThreadNum()) {
...
__global__ void kerSplitPointsExactSoS(int *vertTriArr, int *triToVert,
                                       Tri *triArr, int *insTriMap,
                                       int *exactCheckArr, int *counter,
                                       int triNum, int insTriNum) {
  ...
  for (int idx = getCurThreadIdx(); idx < exactNum; idx += getThreadNum()) {
    ...
    splitPoints<false>(vertIdx, vertTriArr[vertIdx], triToVert, triArr,
                       insTriMap, triNum, insTriNum);
  }
}
...
checkDelaunayFast(int *actTriArr, Tri *triArr, TriOpp *oppArr, char *triInfoArr,
                  int *triVoteArr, int2 *exactCheckVi, int actTriNum,
                  int *counter, int *dbgCircleCountArr) {
  // Iterate active triangle
...
__global__ void kerCheckDelaunayFast(int *actTriArr, Tri *triArr,
                                     TriOpp *oppArr, char *triInfoArr,
                                     int *triVoteArr, int actTriNum,
                                     int *dbgCircleCountArr) {
  checkDelaunayFast<CircleFastOrientFast>(actTriArr, triArr, oppArr, triInfoArr,
                                          triVoteArr, NULL, actTriNum, NULL,
                                          dbgCircleCountArr);
  return;
}

__global__ void kerCheckDelaunayExact_Fast(int *actTriArr, Tri *triArr,
                                           TriOpp *oppArr, char *triInfoArr,
                                           int *triVoteArr, int2 *exactCheckVi,
                                           int actTriNum, int *counter,
                                           int *dbgCircleCountArr) {
  checkDelaunayFast<CircleExactOrientSoS>(actTriArr, triArr, oppArr, triInfoArr,
                                          triVoteArr, exactCheckVi, actTriNum,
                                          counter, dbgCircleCountArr);
  return;
}

__global__ void kerCheckDelaunayExact_Exact(Tri *triArr, TriOpp *oppArr,
                                            int *triVoteArr, int2 *exactCheckVi,
                                            int *counter,
                                            int *dbgCircleCountArr) {
  const int exactNum = counter[CounterExact];
...
template <bool doFast>
__forceinline__ __device__ bool
relocatePoints(int vertex, int &location, int *triToFlip, FlipItem *flipArr) {
  ...
  while (flag == 1) {
    ...
    const Orient ord = doFast ? dPredWrapper.doOrient2DFast(
...
__global__ void kerRelocatePointsFast(KerIntArray vertTriVec, int *triToFlip,
                                      FlipItem *flipArr, int *exactCheckArr,
                                      int *counter) {
  // Iterate points
  for (int idx = getCurThreadIdx(); idx < vertTriVec._num;
       idx += getThreadNum()) {
    bool ret =
        relocatePoints<true>(idx, vertTriVec._arr[idx], triToFlip, flipArr);
...
__global__ void kerRelocatePointsExact(int *vertTriArr, int *triToFlip,
                                       FlipItem *flipArr, int *exactCheckArr,
                                       int *counter) {
  const int exactNum = counter[CounterExact];
...
__forceinline__ __device__ int initPointLocation(int idx, Tri tri) {
  ...
  const int triVert[5] = {tri._v[0], tri._v[1], tri._v[2],
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    Orient ort =
        (doFast)
            ...
            : dPredWrapper.doOrient2DFastExactSoS(triVert[fv[0]],
                                                  triVert[fv[1]], idx);
...
__global__ void kerInitPointLocationFast(KerIntArray vertTriVec,
                                         int *exactCheckArr, int *counter,
                                         Tri tri) {
  // Iterate points
  for (int idx = getCurThreadIdx(); idx < vertTriVec._num;
...
__global__ void kerInitPointLocationExact(int *vertTriArr, int *exactCheckArr,
                                          int *counter, Tri tri) {
  const int exactNum = counter[CounterExact];
...
__global__ void kerVoteForPoint(KerIntArray vertexTriVec, Tri *triArr,
                                int *vertCircleArr, int *triCircleArr,
                                int noSample) {
...
template <bool doFast>
__forceinline__ __device__ void
markTriConsIntersection(KerIntArray actConsVec, Segment *constraintArr,
                        Tri *triArr, TriOpp *oppArr, char *triInfoArr,
                        int *vertTriArr, int *triConsArr, int *counter) {
  // Iterate the constraints
  for (int idx = getCurThreadIdx(); idx < actConsVec._num;
       idx += getThreadNum()) {
    ...
    while (true) {
      ...
      if (nextVert == constraint._v[1]) {
        ...
        or2 = doFast ? dPredWrapper.doOrient2DFast(constraint._v[0],
                                                   constraint._v[1], nextVert)
                     : dPredWrapper.doOrient2DFastExactSoS(
                           constraint._v[0], constraint._v[1], nextVert);

        if (doFast && or2 == OrientZero)
          break;

        if (or1 == OrientNeg && or2 == OrientPos) {
          const int label = encode_constraint(
              consIdx, vi, 2); // side = 2 --> starting triangle
    ...
    // Found the first triangle, walking to find the rest of the intersected
    // ones
    ...
    while (side != 3) {
      ...
      if (nextVert == constraint._v[1]) // Reach the end
      {
        ...
        Orient ori = doFast ? dPredWrapper.doOrient2DFast(
                                  constraint._v[0], constraint._v[1], nextVert)
                            : dPredWrapper.doOrient2DFastExactSoS(
                                  constraint._v[0], constraint._v[1], nextVert);
...
__global__ void
kerMarkTriConsIntersectionFast(KerIntArray actConsVec, Segment *constraintArr,
                               Tri *triArr, TriOpp *oppArr, char *triInfoArr,
                               int *vertTriArr, int *triConsArr, int *counter) {
  markTriConsIntersection<true>(actConsVec, constraintArr, triArr, oppArr,
                                triInfoArr, vertTriArr, triConsArr, counter);
}

__global__ void kerMarkTriConsIntersectionExact(
    KerIntArray actConsVec, Segment *constraintArr, Tri *triArr, TriOpp *oppArr,
    char *triInfoArr, int *vertTriArr, int *triConsArr, int *counter) {
  markTriConsIntersection<false>(actConsVec, constraintArr, triArr, oppArr,
                                 triInfoArr, vertTriArr, triConsArr, counter);
}

template <bool doFast>
__forceinline__ __device__ bool updatePairStatus(int triIdx, int *triConsVec,
                                                 Tri *triArr, TriOpp *oppArr,
                                                 char *triInfoArr) {
  if (doFast) {
  ...
  Orient or1 = doFast ? dPredWrapper.doOrient2DFast(tri._v[vi], oppVert,
                                                    tri._v[(vi + 1) % 3])
                      : dPredWrapper.doOrient2DFastExactSoS(
                            tri._v[vi], oppVert, tri._v[(vi + 1) % 3]);
  Orient or2 = doFast ? dPredWrapper.doOrient2DFast(tri._v[vi], oppVert,
                                                    tri._v[(vi + 2) % 3])
                      : dPredWrapper.doOrient2DFastExactSoS(
                            tri._v[vi], oppVert, tri._v[(vi + 2) % 3]);
...
__global__ void kerUpdatePairStatusFast(KerIntArray actTriVec, int *triConsVec,
                                        Tri *triArr, TriOpp *oppArr,
                                        char *triInfoArr, int *exactArr,
                                        int *counter) {
  // Iterate active triangles
  for (int idx = getCurThreadIdx(); idx < actTriVec._num;
       idx += getThreadNum()) {
    const int triIdx = actTriVec._arr[idx];

    bool ret =
        updatePairStatus<true>(triIdx, triConsVec, triArr, oppArr, triInfoArr);
...
__global__ void kerUpdatePairStatusExact(KerIntArray actTriVec, int *triConsVec,
                                         Tri *triArr, TriOpp *oppArr,
                                         char *triInfoArr, int *exactArr,
                                         int *counter) {
  const int exactNum = counter[CounterExact];
...
template <bool doFast>
__forceinline__ __device__ bool
checkConsFlipping(int midIdx, int *triConsArr, char *triInfoArr, Tri *triArr,
                  TriOpp *oppArr, int *triVoteArr) {
  PairType midType;
  ...
  if (doFast &&
      (leftLabel < 0 || decode_cIdx(leftLabel) != decode_cIdx(midLabel)))
    return true;
  ...
  Orient or1 =
      doFast ? dPredWrapper.doOrient2DFast(tri._v[leftVi], rightVert,
                                           tri._v[(leftVi + 1) % 3])
             : dPredWrapper.doOrient2DFastExactSoS(tri._v[leftVi], rightVert,
                                                   tri._v[(leftVi + 1) % 3]);
  Orient or2 =
      doFast ? dPredWrapper.doOrient2DFast(tri._v[leftVi], rightVert,
                                           tri._v[(leftVi + 2) % 3])
...
__global__ void kerCheckConsFlippingFast(KerIntArray actTriVec, int *triConsArr,
                                         char *triInfoArr, Tri *triArr,
                                         TriOpp *oppArr, int *triVoteArr,
                                         int *exactArr, int *counter) {
  // Iterate active triangles
  for (int idx = getCurThreadIdx(); idx < actTriVec._num;
       idx += getThreadNum()) {
    const int triIdx = actTriVec._arr[idx];

    bool ret = checkConsFlipping<true>(triIdx, triConsArr, triInfoArr, triArr,
                                       oppArr, triVoteArr);
...
__global__ void kerCheckConsFlippingExact(KerIntArray actTriVec,
                                          int *triConsArr, char *triInfoArr,
                                          Tri *triArr, TriOpp *oppArr,
                                          int *triVoteArr, int *exactArr,
                                          int *counter) {
  const int exactNum = counter[CounterExact];
  ...
  for (int idx = getCurThreadIdx(); idx < exactNum; idx += getThreadNum()) {
    ...
    checkConsFlipping<false>(triIdx, triConsArr, triInfoArr, triArr, oppArr,
                             triVoteArr);
  }
}
