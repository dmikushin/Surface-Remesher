...
template <typename T> class DevVector {
public:
  // Types
  ...
  void expand(size_t n) {
    assert((_capacity >= n) &&
           "New size not within current capacity! Use resize!");
    _size = n;
  }
  ...
  void resize(size_t n) {
    ...
    if (!_owned && _capacity > 0) {
      std::cerr << "WARNING: Resizing a DevVector with borrowing pointer!"
                << std::endl;
    }
    ...
    try {
      ...
      std::cerr << "thrust::device_malloc failed to allocate "
                << (sizeof(T) * _capacity) / OneMB << " MB!" << std::endl;
      std::cerr << "size = " << _size << " sizeof(T) = " << sizeof(T)
                << std::endl;
      exit(-1);
    }
...
public:
  ...
  void free(bool report = false) {
    for (int i = 0; i < _memPool.size(); ++i) {
      if (report)
        std::cout << "MemoryPool: [" << i << "]" << _memPool[i].sizeInBytes
                  << std::endl;
  ...
  template <typename T> int reserve(size_t size) {
  ...
  DevVector<T> allocateAny(size_t size, bool tempOnly = false) {
    ...
    for (int i = 0; i < _memPool.size(); ++i)
      if (_memPool[i].avail && _memPool[i].sizeInBytes >= sizeInBytes)
        if (bufIdx == -1 ||
            _memPool[i].sizeInBytes < _memPool[bufIdx].sizeInBytes)
          bufIdx = i;
    ...
    //     << sizeInBytes << ", giving " << _memPool[ bufIdx ].sizeInBytes <<
    //     std::endl;
  ...
  template <typename T> void release(DevVector<T> &vec) {
    for (int i = 0; i < _memPool.size(); ++i)
      if (_memPool[i].ptr == (void *)vec._ptr.get()) {
        ...
        // std::cout << "MemoryPool: Returning " << _memPool[i].sizeInBytes <<
        // std::endl;
    ...
    std::cerr << "WARNING: Releasing a DevVector not in the MemoryPool!"
              << std::endl;
