...
struct __align__(8) RealType2 { float _c0, _c1; };
#else
struct __align__(16) RealType2 { double _c0, _c1; };
#endif

__forceinline__ __device__ Point2 loadPoint2(const Point2 *pointArr, int idx) {
  const RealType2 ptxy = ((RealType2 *)pointArr)[idx];
  const Point2 pt = {ptxy._c0, ptxy._c1};

  return pt;
}

__forceinline__ __device__ FlipItemTriIdx loadFlipTriIdx(FlipItem *flipArr,
                                                         int idx) {
...
__forceinline__ __device__ void storeFlip(FlipItem *flipArr, int idx,
                                          const FlipItem &item) {
...
__forceinline__ __host__ __device__ int roundUp(int num, int div) {
...
__forceinline__ __device__ void voteForFlip(int *triVoteArr, int botTi,
                                            int topTi, int botVi) {
...
__forceinline__ __device__ int decode_cIdx(int label) { return (label >> 4); }

__forceinline__ __device__ int decode_cVi(int label) {
  return (label >> 2) & 3;
}

__forceinline__ __device__ int decode_cSide(int label) { return (label & 3); }
