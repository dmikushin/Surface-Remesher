...
__device__ int d_scale_expansion_zeroelim(const RealType *predConsts, int elen,
                                          RealType *e, RealType b,
                                          RealType *h) {
  RealType Q, sum;
  RealType hh;
  RealType product1;
...
__forceinline__ __device__ RealType orient2dExact(const RealType *predConsts,
                                                  const RealType *pa,
                                                  const RealType *pb,
                                                  const RealType *pc) {
...
__device__ RealType orient2dFast(const RealType *predConsts, const RealType *pa,
                                 const RealType *pb, const RealType *pc) {
...
__device__ RealType orient2dDet(const RealType *predConsts, const RealType *pa,
                                const RealType *pb, const RealType *pc) {
...
__device__ RealType orient2dFastExact(const RealType *predConsts,
                                      const RealType *pa, const RealType *pb,
                                      const RealType *pc) {
  RealType detleft, detright, det;
  RealType detsum, errbound;
...
__noinline__ __device__ int calc_det(const RealType *predConsts, RealType *a,
  ...
  int xlen = d_scale_twice_expansion_zeroelim(predConsts, temp3len, temp3, fx0,
                                              fx1, detx);
  int ylen = d_scale_twice_expansion_zeroelim(predConsts, temp3len, temp3, fy0,
                                              fy1, dety);

  return d_fast_expansion_sum_zeroelim(xlen, detx, ylen, dety, ret);
}

__device__ RealType incircleExact(const RealType *predConsts,
                                  const RealType *pa, const RealType *pb,
                                  const RealType *pc, const RealType *pd) {
...
__noinline__ __device__ int calc_det_adapt(const RealType *predConsts,
                                           RealType adx, RealType ady,
                                           RealType bdx, RealType bdy,
                                           RealType cdx, RealType cdy,
                                           RealType *temp4, RealType *temp16x,
                                           RealType *temp16y, RealType *ret) {
  RealType axby1, axby0, bxay1, bxay0;
  ...
  Two_Two_Diff(axby1, axby0, bxay1, bxay0, temp4[3], temp4[2], temp4[1],
               temp4[0]);

  int temp16xlen =
      d_scale_twice_expansion_zeroelim(predConsts, 4, temp4, cdx, cdx, temp16x);
  int temp16ylen =
      d_scale_twice_expansion_zeroelim(predConsts, 4, temp4, cdy, cdy, temp16y);

  return d_fast_expansion_sum_zeroelim(temp16xlen, temp16x, temp16ylen, temp16y,
                                       ret);
}

__device__ RealType incircleAdaptExact(const RealType *predConsts,
                                       const RealType *pa, const RealType *pb,
                                       const RealType *pc, const RealType *pd,
  ...
  alen = calc_det_adapt(predConsts, bdx, bdy, cdx, cdy, adx, ady, temp4,
                        temp16x, temp16y, adet);
  blen = calc_det_adapt(predConsts, cdx, cdy, adx, ady, bdx, bdy, temp4,
                        temp16x, temp16y, bdet);

  ablen = d_fast_expansion_sum_zeroelim(alen, adet, blen, bdet, abdet);

  clen = calc_det_adapt(predConsts, adx, ady, bdx, bdy, cdx, cdy, temp4,
                        temp16x, temp16y, cdet);
...
__forceinline__ __device__ RealType incircleFastAdaptExact(
    const RealType *predConsts, const RealType *pa, const RealType *pb,
    const RealType *pc, const RealType *pd) {
  RealType adx, bdx, cdx, ady, bdy, cdy;
  ...
  return incircleAdaptExact(predConsts, pa, pb, pc, pd,
                            permanent); // Needs exact predicate
}

__forceinline__ __device__ RealType incircleFast(const RealType *predConsts,
                                                 const RealType *pa,
                                                 const RealType *pb,
                                                 const RealType *pc,
                                                 const RealType *pd) {
...
__forceinline__ __device__ RealType incircleDet(const RealType *predConsts,
                                                const RealType *pa,
                                                const RealType *pb,
                                                const RealType *pc,
                                                const RealType *pd) {
...
__device__ RealType orient1dExact_Lifted(const RealType *predConsts,
                                         const RealType *pa,
                                         const RealType *pb) {
  ...
  Two_Two_Diff(axax1, axax0, bxbx1, bxbx0, aterms[3], aterms[2], aterms[1],
               aterms[0]);

  Two_Product(pa[1], pa[1], ayay1, ayay0);
  Two_Product(pb[1], pb[1], byby1, byby0);
  Two_Two_Diff(ayay1, ayay0, byby1, byby0, bterms[3], bterms[2], bterms[1],
               bterms[0]);

  return d_fast_expansion_sum_sign(4, aterms, 4, bterms);
}

__device__ RealType orient2dExact_Lifted(const RealType *predConsts,
                                         const RealType *pa, const RealType *pb,
                                         const RealType *pc, bool lifted) {
  ...
  if (lifted) {
    ...
    Two_Two_Diff(aax1, aax0, aay1, aay0, palift[3], palift[2], palift[1],
                 palift[0]);
    palen = 4;

    Two_Product(pb[0], pb[0], aax1, aax0);
    Two_Product(-pb[1], pb[1], aay1, aay0);
    Two_Two_Diff(aax1, aax0, aay1, aay0, pblift[3], pblift[2], pblift[1],
                 pblift[0]);
    pblen = 4;

    Two_Product(pc[0], pc[0], aax1, aax0);
    Two_Product(-pc[1], pc[1], aay1, aay0);
    Two_Two_Diff(aax1, aax0, aay1, aay0, pclift[3], pclift[2], pclift[1],
                 pclift[0]);
    pclen = 4;
  } else {
    palen = 1;
  ...
  xy1len =
      d_scale_expansion_zeroelim(predConsts, pblen, pblift, pa[0], xy1terms);
  xy2len =
      d_scale_expansion_zeroelim(predConsts, pclen, pclift, -pa[0], xy2terms);
  alen =
      d_fast_expansion_sum_zeroelim(xy1len, xy1terms, xy2len, xy2terms, aterms);

  xy1len =
      d_scale_expansion_zeroelim(predConsts, pclen, pclift, pb[0], xy1terms);
  xy2len =
      d_scale_expansion_zeroelim(predConsts, palen, palift, -pb[0], xy2terms);
  blen =
      d_fast_expansion_sum_zeroelim(xy1len, xy1terms, xy2len, xy2terms, bterms);

  vlen = d_fast_expansion_sum_zeroelim(alen, aterms, blen, bterms, v);

  xy1len =
      d_scale_expansion_zeroelim(predConsts, palen, palift, pc[0], xy1terms);
  xy2len =
      d_scale_expansion_zeroelim(predConsts, pblen, pblift, -pc[0], xy2terms);
  clen =
      d_fast_expansion_sum_zeroelim(xy1len, xy1terms, xy2len, xy2terms, cterms);
