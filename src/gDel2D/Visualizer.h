...
template <typename T> struct DiffItem {
  int idx;
  T left;
  T right;
};

template <typename T> struct DiffStorage {
  int sizeLeft;
  int sizeRight;
  ...
  int sizeInBytes() { return 8 + diffVec.capacity() * sizeof(DiffItem<T>); }

  void create(const thrust::host_vector<T> &prevVec,
...
private:
  ...
  template <typename T>
  void computeDiff(const thrust::host_vector<T> &prevVec,
                   const thrust::host_vector<T> &nextVec,
...
public:
  ...
  void addFrame(const Point2HVec &pointVec, const SegmentHVec &constraintVec,
                const TriHVec &triVec, const IntHVec &triColorVec,
                int infIdx = -1);
  void addFrame(const Point2HVec &pointVec, const SegmentHVec &constraintVec,
                const TriHVec &triVec, int infIdx = -1);
  void addFrame(const Point2DVec &pointVec, const TriDVec &triVec,
                int infIdx = -1);
  void addFrame(const Point2DVec &pointVec, const SegmentDVec &constraintVec,
                const TriDVec &triVec, int infIdx = -1);
  void addFrame(const Point2DVec &pointVec, const SegmentDVec &constraintVec,
